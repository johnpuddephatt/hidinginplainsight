  ...
  >
    ...
    <v-marker-cluster :options="clusterOptions">
      <l-marker
        ...
        :key="cinema.slug"
        :lat-lng="getLatLng(cinema.location.coordinates)"
        @click="markerClicked($event, cinema.slug)"
      >
        <l-icon
          ...
          :icon-url="
            clicked == cinema.slug
              ? '/assets/images/marker-icon-red.svg'
              : '/assets/images/marker-icon.svg'
          "
          :icon-retina-url="
            clicked == cinema.slug
              ? '/assets/images/marker-icon-red.svg'
    ...
    <l-marker
      ...
      :key="stop.slug"
      :lat-lng="getLatLng(stop.location.coordinates)"
      @click="markerClicked($event, stop.slug)"
    >
    ...
    <l-marker
      ...
      v-for="point in poi"
      :key="point.slug"
      :lat-lng="getLatLng(point.location.coordinates)"
    >
      <l-popup :options="{ offset: [0, -34], closeButton: false }">
        ...
        <div class="popup-header">
          ...
          <p
            class="cinema-address"
            v-if="point.address"
            v-html="point.address"
          ></p>
          <div
            class="cinema-description is-small"
      ...
      <l-icon
        icon-url="/assets/images/star-icon.svg"
        icon-retina-url="/assets/images/star-icon.svg"
        :iconSize="[40, 70]"
      />
    </l-marker>
  </l-map>
...
export default {
  ...
  data() {
    return {
      ...
      center: this.is_tour
        ? latLng(53.7993475, -1.5432696)
        : latLng(53.8125403, -1.5735477),
      ...
      clusterOptions: {
        ...
        iconCreateFunction: (cluster) => {
          ...
          return new DivIcon({
            ...
            className:
              "marker-cluster my-marker-cluster-" +
              (childCount < 6 ? "small" : "large"),
  ...
  watch: {
    clicked: function(clicked) {
      if (
        (this.$router.currentRoute.name == "cinemas" ||
          this.$router.currentRoute.name == "tour") &&
        clicked
      ) {
