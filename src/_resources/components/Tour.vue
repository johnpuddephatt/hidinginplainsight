<template>
  <div class="map-wrapper">
    ...
    <transition name="popup">
      <Popup :secondary_popup_active="audio_active || nextStop" :is_tour="true" v-for="cinema in tour.cinemas" :cinema="cinema" @close="clicked = null" @start-audio="onStartAudio" v-if="clicked == cinema.slug" :key="cinema.slug"></Popup>
    </transition>

    <transition name="popup">
      <Popup :secondary_popup_active="audio_active || nextStop" :is_tour="true" v-for="stop in tour.stops" :cinema="stop" @close="clicked = null" @start-audio="onStartAudio" v-if="clicked == stop.slug" :key="stop.slug"></Popup>
    </transition>

    <transition name="popup">
      <AudioPlayer :key="audio_active.slug" :autoplay="true" v-if="audio_active" playerid="1" @audio-ended="audioEnded" :url="audio_active.audio" :is_tour="true"></AudioPlayer>
    </transition>
...
export default {
  ...
  methods: {
    ...
    findNextStop() {
      let currentStop = this.tour.stops.findIndex(stop => stop.slug == this.clicked);
